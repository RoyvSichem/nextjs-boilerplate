...
export default async function ProfilePage() {
  ...
  // streak via RPC, zonder generics op .single()
  const streakRes = await sb.rpc('get_streak', { p_user_id: user.id }).single();
  const streakRow = (streakRes.data as StreakRow | null) ?? null;

  const currentStreak = streakRow?.current_streak ?? 0;
  const bestStreak = streakRow?.best_streak ?? 0;

  // totaal checkins en laatste
  const { data: lastRows, count: totalCheckins = 0 } = await sb
    .from('checkins')
  ...
  // favorieten ids, geen generics op .from()
  const favIdsRes = await sb
    .from('favorites')
    .select('meditation_id')
    .eq('user_id', user.id);

  const favIds = (favIdsRes.data as FavIdRow[] | null) ?? [];

  let favorites: FavMeditation[] = [];
  if (favIds.length) {
    const ids = favIds.map(f => f.meditation_id);
    const medsRes = await sb
      .from('meditations')
      .select('id, slug, title, cover_url, duration_seconds')
      .in('id', ids);

    favorites = (medsRes.data as FavMeditation[] | null) ?? [];
  }
      ...
      <div className="grid cards" style={{ marginTop: 12 }}>
        <section className="card">
          ...
          <p style={{ margin: '8px 0' }}>
            Huidig: <b>{currentStreak} dag{currentStreak === 1 ? '' : 'en'}</b><br />
          ...
          <Link href="/checkin" className="btn" style={{ marginTop: 8, display: 'inline-block' }}>
            Check nu in
          </Link>
        </section>
        ...
        <section className="card">
          ...
          <p style={{ margin: '8px 0' }}>
            Laatste checkin, <b>{lastCheckin ? new Date(lastCheckin).toLocaleDateString() : '—'}</b>
          </p>
              ...
              <article key={f.id} className="card" style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: 'var(--muted)', fontSize: 13 }}>
                    {f.duration_seconds ? `${Math.round(f.duration_seconds / 60)} min` : ''}
                  </div>
                </div>
