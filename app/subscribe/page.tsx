...
export default function Subscribe(){
  const go = async ()=>{ const r = await fetch('/api/checkout', { method:'POST' }); const { url } = await r.json(); window.location.href = url; };
  return (
    <main style={{maxWidth:560, margin:'40px auto', padding:'0 16px', textAlign:'center'}}>
      <h1>Word lid</h1>
      <p>Onbeperkt meditaties voor €4,95 per maand</p>
      <button onClick={go} className="btn">Naar betalen</button>
