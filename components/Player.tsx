...
export default function Player({ src, duration, onComplete }:{
  src:string, duration?:number, onComplete?:(seconds:number)=>void
}){
  ...
  useEffect(()=>{
    ...
    const onEnd=()=>onComplete?.(Math.floor(el.currentTime));
    el.addEventListener('timeupdate',onTime);
    el.addEventListener('ended',onEnd);
    return ()=>{ el.removeEventListener('timeupdate',onTime); el.removeEventListener('ended',onEnd); };
  },[onComplete]);

  const seek=(d:number)=>{ const el=a.current; if(!el) return; el.currentTime=Math.max(0, Math.min((el.duration||duration||0), el.currentTime+d)); };
  ...
  return (
    <div>
      ...
      <div style={{display:'flex', alignItems:'center', gap:12, margin:'12px 0'}}>
        <span>{fmt(t)}</span>
        <input type="range" min={0} max={duration||0} value={t} onChange={e=>{ const el=a.current; if(el){ el.currentTime=Number(e.target.value);} }} style={{flex:1}} />
      ...
      <div style={{display:'flex', justifyContent:'space-around', alignItems:'center', gap:8}}>
        ...
        <button onClick={()=>{ const el=a.current; if(!el) return; el.paused?el.play():el.pause(); }} className="btn">▶︎⏸</button>
        <button onClick={()=>seek(30)} className="btn">30 ⏩</button>
